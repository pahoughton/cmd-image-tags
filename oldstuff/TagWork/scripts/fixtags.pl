...
while( <> ) {
  ...
  if( defined( $srcTags->{ XPComment } ) ) {
    $capt = $srcTags->{ XPComment };
  }

  print "exiftool -DateTimeOriginal='$dt' -overwrite_original  -City=$city -Province-State=$state -Location=\"$location\" -Keywords=Houghton -ObjectName=\"$title\" -Caption-Abstract=\"$capt\" \"$_\"\n";
